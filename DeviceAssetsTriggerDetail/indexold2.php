...
if (USER_ROLE !== 1) {
    ...
    if (!empty($invalidIdArr)) {
        errorInvalid("NO_PERMISSION_FOR_ASSET_ID_(" . implode('_', $invalidIdArr) . ")", API, $logParent);
    }
}
...
if (empty($deviceAssetDet)) {
    errorInvalid("NO_DEVICE_FOR_ASSET", API, $logParent);
}

$linkedAssetId = array_keys($deviceAssetDet);
$invalidIdArr = array_diff($sanitisedInput['asset_id'], $linkedAssetId);
if (!empty($invalidIdArr)) {
    errorInvalid("NO_DEVICE_FOR_ASSET_ID_(" . implode('_', $invalidIdArr) . ")", API, $logParent);
}
...
if ($sanitisedInput['action'] === "insert") {
    if (count($sanitisedInput['trigger_id']) !== count(array_merge($sensorTriggerIdArr, $geofenceTriggerIdArr))) {
        errorInvalid("CONTAIN_NONE_EXIST_TRIGGER_ID", API, $logParent);
    }
    ...
    if (!empty($sensorTriggerIdArr)) {
        ...
        array_walk(
            ...
            function($provisioningId, $deviceId) use ($triggerSdIdArr, $provisioningSensorDet, $triggerSensorDef) {
                ...
                if (!empty($invalidTriggerIdArr)) {
                    errorInvalid("TRIGGER_ID_(" . implode('_', $invalidTriggerIdArr) . ")_FOR_ASSET", $API, $logParent);
                }
            }
        );
