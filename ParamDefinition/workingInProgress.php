...
if ($sanitisedInput['action'] == "select"){
  ...
  elseif ($sanitisedInput['action'] == "insert"){
    ...
    if (isset($inputarray['param_name'])) {
      $insertArray['param_name'] = sanitise_input($inputarray['param_name'], "param_name", $schemainfoArray['param_name'], $API, $logParent);
      $sql = "SELECT param_name 
                FROM param_def 
                WHERE param_name = '". $insertArray['param_name'] ."'";
        $stm = $pdo->query($sql);
        $rows = $stm->fetchAll(PDO::FETCH_NUM);
        if (isset($rows[0][0])) {
            errorInvalid("param_name", $API, $logParent);
        }
    }
    else {
  ...
  elseif($sanitisedInput['action'] == "update"){
      ...
      if (isset($inputarray['param_name'])) 
      {
        ...
        $sql = "SELECT param_name 
                FROM param_def 
                WHERE param_name = '". $updateArray['param_name'] ."'";
        $stm = $pdo->query($sql);
        $rows = $stm->fetchAll(PDO::FETCH_NUM);
        if (isset($rows[0][0])) {
            errorInvalid("param_name", $API, $logParent);
